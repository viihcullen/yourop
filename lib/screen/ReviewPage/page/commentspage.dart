                  ...
                  Row(
                    children: [
                      ...
                      IconButton(
                        ...
                        onPressed: () async {
                          var idUsuario = json.decode((await API.getUsuario(FirebaseAuth.instance.currentUser!.uid)).body)['idUsuario'];
                          var response = await API.cadastrarComentario({'idObra': widget.content['idObra'], 'idUsuario': idUsuario, 'valorAvaliacao': ratingComment, 'comentarioAvaliacao': _commentController.text});
                          if(response.statusCode == 200){
                            _commentController.text = "";
                            var newObra = json.decode((await API.getObra(widget.content['idObra'])).body);
                            Navigator.pushReplacement(context, MaterialPageRoute(builder: (context)=> ReviewPage(obra: newObra)));
                          }
